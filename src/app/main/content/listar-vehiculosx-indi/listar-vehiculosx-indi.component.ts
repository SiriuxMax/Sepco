...
@Component({
    moduleId: module.id,
    selector: 'listar-vehiculosx-indi',
    templateUrl: 'listar-vehiculosx-indi.component.html',
    styleUrls: ['listar-vehiculosx-indi.component.scss']
})
export class ListarVehiculosxIndiComponent implements OnInit {
    rows = [];
    public directordepto:E_Individuo2;
    ...
    ObtenerReuniones() {
        ...
        var Objdire: E_Individuo2 = new E_Individuo2()
        var xxx = this.userservices.GetCurrentCurrentUserNow().UserName
        Objdire.Correo = xxx;
        this.MetasService.Individuo2xCorreo(Objdire).subscribe((x) => {
            
            this.directordepto = x;            
        });

        var ObjReu: E_Vehiculo = new E_Vehiculo()
        ObjReu.Id_individuo2 = this.directordepto.Id;
        this.MetasService.listarVehiculoxIndi(ObjReu).subscribe((x) => {
