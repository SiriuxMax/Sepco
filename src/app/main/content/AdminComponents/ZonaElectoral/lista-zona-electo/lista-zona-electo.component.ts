...
@Component({
    moduleId: module.id,
    selector: 'lista-zona-electo',
    templateUrl: 'lista-zona-electo.component.html',
    styleUrls: ['lista-zona-electo.component.scss']
})
export class ListaZonaElectoComponent implements OnInit {
    rows = [];
    ...
    ObtenerReuniones() {
        ...
        this.ParameterService.ListarMunicipios().subscribe((x: Array<E_Municipios>) => {
            ...
            this.ImageService.listarZonas().subscribe((x) => {
                debugger;
                x.forEach(element => {
                    if(this.ListMunicipiosBase.find( (x) => x.Id == element.Id_Municipio) !=  undefined){
                                element.NombreMuni=this.ListMunicipiosBase.find( (x) => x.Id == element.Id_Municipio).Nombre
                    }
    ...
    nuevo(){
        this.Router.navigate(['/zonaelectoral'])
    }
}
