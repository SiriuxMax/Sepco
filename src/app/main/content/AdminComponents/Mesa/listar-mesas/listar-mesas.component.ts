...
export class ListarMesasComponent implements OnInit {
    ...
    ObtenerReuniones() {
        ...
        this.ParameterService.listarPuestosVotacion().subscribe((x: Array<E_PuestoVotacion>) => {
            debugger;
            this.ListMunicipiosBase = x
            this.ImageService.listarMesas().subscribe((x) => {
                debugger;
                x.forEach(element => {
                    if(this.ListMunicipiosBase.find( (x) => x.Id == element.Id_puestoVotacion) !=  undefined){
                                element.NombrePV=this.ListMunicipiosBase.find( (x) => x.Id == element.Id_puestoVotacion).Nombre
                    }
